...
| Sink-scoped idempotency (canonical fingerprints) + persist-before-memory proposal transitions (apply uses two non-atomic storage keys) + persisted active `UiDocument` (workspace/artifact-scoped, fail-closed reload incl. stored-record bounds) + protected governance surfaces (`review-table`/`approval-list`/`ui-builder` cannot be removed; code `protected_surface`) + no-Host behavior | implemented + unit-tested |
